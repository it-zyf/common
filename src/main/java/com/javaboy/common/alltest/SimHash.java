...
//        System.out.println(hash1.hammingDistance(hash3) + " " + dis2);
        ...
        String json1="亲爱的朋友们，大家早上好";
        SimHash one = new SimHash(json1, 64);
        one.subByDistance(one, 3);
        String json2="亲爱的朋友们，大家晚上好";
        SimHash two = new SimHash(json2, 64);

        two.subByDistance(two,3);
        System.out.println(one.strSimHash);
        System.out.println(two.strSimHash);
        System.out.println(two.intSimHash + "  " + one.intSimHash);
        // 计算 海明距离 在 3 以内的各块签名的 hash 值



        int distance = two.getDistance(two.strSimHash, one.strSimHash);
